...
public interface onlineDao {
	...
	String selectAllCate = "SELECT RNUM, ONLINE_BOARD_ID, NICKNAME, CATEGORY_ID, ONLINE_TITLE, PRICE_SAT, "
			+ "PRODUCT_SAT, RECOMD, HITS, CATEGORY_NAME, ADD_PRODUCT  "
			+ "FROM( SELECT ROWNUM AS RNUM, ONLINE_BOARD_ID, NICKNAME, CATEGORY_ID, ONLINE_TITLE, PRICE_SAT, "
			+ "PRODUCT_SAT, RECOMD, HITS, CATEGORY_NAME,  ADD_PRODUCT FROM( "
			+ "SELECT ONLINE_BOARD_ID, NICKNAME, OB.CATEGORY_ID, ONLINE_TITLE, PRICE_SAT, PRODUCT_SAT, RECOMD, HITS, "
			+ "CATEGORY_NAME,  ADD_PRODUCT FROM ONLINE_BOARD OB JOIN ONLINE_CATEGORY OC ON(OB.CATEGORY_ID=OC.CATEGORY_ID) "
			+ "WHERE OB.CATEGORY_ID=? AND DELETE_N=0 AND STATUS=1)WHERE ROWNUM<?) WHERE RNUM>=?";
	
//	String onlineRowQuery = " SELECT COUNT(*) FROM ONLINE_BOARD WHERE CATEGORY_ID=? AND DELETE_N=0 AND STATUS=1 ";
	//String onlineRowQuery = "SELECT COUNT(*) FROM ONLINE_BOARD WHERE CATEGORY_ID=?";
	String onlineRowQuery = " SELECT COUNT(*) FROM ONLINE_BOARD WHERE CATEGORY_ID=?";
	
	//단일 게시글 조회
	String onlineSelectQuery = "SELECT ONLINE_BOARD_ID, NICKNAME, OB.CATEGORY_ID, ONLINE_TITLE, ONLINE_CONTENT, PRICE_SAT,PRODUCT_SAT, ADD_RECEIPT, CREATEAT, STATUS, RECOMD,HITS,CATEGORY_NAME,ADD_PRODUCT FROM ONLINE_BOARD OB JOIN ONLINE_CATEGORY OC ON(OB.CATEGORY_ID = OC.CATEGORY_ID) WHERE ONLINE_BOARD_ID=? AND DELETE_N=0";
